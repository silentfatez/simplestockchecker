...
url = 'http://financials.morningstar.com/valuate/current-valuation-list.action?&t=XNAS:'+ticker+'&region=usa&culture=en-US&cur=&adsFlag=true&_=1596023281217'
...
try:
    ...
    try:##doesnt work for Visa need to use selenium maybe in the future
        url='http://financials.morningstar.com/valuate/current-valuation-list.action?&t=XNYS:'+ticker+'&region=usa&culture=en-US&cur=&adsFlag=true&_=1596023281217'
        source = urllib.request.urlopen(url).read()
        soup = BeautifulSoup(source, "lxml")
        table = soup.find("table", attrs={"class": "r_table1 text2"})
        df = pd.read_html(str(table))[0]
